...
{
    @Test
    public void readIntTypeDeltaKernelPOC()
    {
    ...
    @Test
    public void readDVTableDeltaKernelPOC()
    {
        // Test reading following primitive types from a Delta table (all ints, float, double, decimal, boolean, varchar, varbinary)
        String path = goldenTablePath("basic-dv-no-checkpoint");
        String testQuery =
                format("SELECT * FROM \"%s\".\"%s\"", PATH_SCHEMA, path);
        // H2 query doesn't work for VALUES with multiple rows, use UNION ALL
...
//    }
    ...
    @Test(enabled = false) // Enabled onces snapshot selection is supported in Delta Kernel
    ...
    @Test(enabled = false) // Enable once the TrinoColumnVector.getMap/getStruct/getArray is implemented.
    ...
    @Test
    public void testKernelStandalone()
            throws Exception
    {
        Configuration hadoopConf = new Configuration();
        DefaultTableClient tableClient = DefaultTableClient.create(hadoopConf);

        Table table = Table.forPath("file:/Users/venkateshwar.korukanti/test-tables/kernel-demo");
        Snapshot snapshot = table.getLatestSnapshot(tableClient);
        StructType tableSchema = snapshot.getSchema(tableClient);

        StructType readSchema = tableSchema;
        ...
        try (
                CloseableIterator<ColumnarBatch> scanFilesIter = scan.getScanFiles(tableClient)) {
            while (scanFilesIter.hasNext()) {
                try (CloseableIterator<DataReadResult> dataIter = Scan.readData(
                        tableClient,
                        scanState,
                        scanFilesIter.next().getRows(),
                        Optional.empty())) {

                    while (dataIter.hasNext()) {
                        DataReadResult dataReadResult = dataIter.next();

                        // Data in columnar format
                        ColumnarBatch dataBatch = dataReadResult.getData();

                        // Optional selection vector to select only a subset of rows from the
                        // above `dataBatch`.
                        Optional<ColumnVector> selectionVector = dataReadResult.getSelectionVector();

                        // ... program logic to read the data and further process
                    }
                }
            }
        }
