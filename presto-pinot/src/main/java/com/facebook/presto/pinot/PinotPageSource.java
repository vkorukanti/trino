        ...
        implements ConnectorPageSource
{
    ...
    public Page getNextPage()
    {
        ...
        for (int columnHandleIdx = 0; columnHandleIdx < columnHandles.size(); columnHandleIdx++) {
            ...
            if (isAggregationPushdownEnabled && !split.getAggregations().containsKey(columnHandles.get(columnHandleIdx).getColumnName().toLowerCase(ENGLISH))) {
        ...
        // TODO (venkik disabled this) pageBuilder.declareIsAgregated(isAggregationPushdownEnabled);
    ...
    private void fetchPinotData()
    {
        ...
        for (PinotColumnHandle columnHandle : columnHandles) {
            if (isAggregationPushdownEnabled) {
                ...
                if (split.getAggregations().containsKey(columnHandle.getColumnName().toLowerCase(ENGLISH))) {
                    columnHandle.setAggregationType(Optional.of(split.getAggregations().get(columnHandle.getColumnName().toLowerCase(ENGLISH)).get(0)));
    ...
    Type getTypeForBlock(boolean shouldPushAggregation, Map<String, List<String>> aggregations, PinotColumnHandle pinotColumnHandle)
    {
        if (pinotColumnHandle.getColumnType().equals(INTEGER)) {
            ...
            String columnNameLower = pinotColumnHandle.getColumnName().toLowerCase(ENGLISH);
